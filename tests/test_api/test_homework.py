...
def test_create_homework_basic(client):
    ...
    assert teacher_response.status_code == 200, f"Failed to create teacher: {teacher_response.json()}"
    ...
    assert student_response.status_code == 200, f"Failed to create student: {student_response.json()}"
    ...
    # When
    response = client.post("/homework/assign/", json=homework_data)  # Note: changed endpoint to /homework/assign/
...
def test_create_homework_multiple_students(client):
    ...
    for i in range(3):
        ...
        student_response = client.post("/users/", json=student_data)
        assert student_response.status_code == 200, f"Failed to create student {i}: {student_response.json()}"
    ...
    assert response.status_code == 200, f"Failed to create homework: {response.text}"
    data = response.json()
    assert len(data["student_ids"]) == 3, f"Expected 3 students, got {len(data['student_ids'])}"
...
def test_get_homework_by_id(client):
    ...
    assert homework_response.status_code == 200, f"Failed to create homework: {homework_response.text}"
...
def test_get_teacher_homework(client):
    ...
    for i in range(3):
        ...
        assert response.status_code == 200, f"Failed to create homework {i}: {response.text}"

    # When
    response = client.get(f"/homework/teacher/{teacher_id}")

    # Then
    assert response.status_code == 200, f"Failed to get teacher homework: {response.text}"
    data = response.json()
...
def test_invalid_teacher_homework(client):
    ...
    assert any(msg in error_detail for msg in ["teacher not found", "not found", "invalid teacher"])
...
def test_get_student_homework_with_status(client):
    ...
    # When
    response = client.get(
        f"/homework/student/{student_id}",
        params={"homework_status": "pending"}
    )
