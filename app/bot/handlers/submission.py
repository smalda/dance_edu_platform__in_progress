...
class SubmissionHandler(BaseHandler):
    async def start_submit(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        markup = create_selection_menu(options, done_button=False)  # No need for done button here
        await update.message.reply_text(
    ...
    async def handle_homework_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def handle_submission(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not homework_id:
            await update.message.reply_text("Something went wrong. Please start over with /submit")
            return ConversationHandler.END
